	...
	go func(ctx context.Context) {
		for {
			...
			case <-ctx.Done():
				...
				return
			case flow := <-inputChan:
				fmt.Println("[FlowQueue] received flow", flow.UUID, "req:", len(flow.Request.GetData()), "resp:", len(flow.Response.GetData()))
				...
				apiFlow := &Flow{
					...
					RequestTxt:  flow.Request.GetData(),
					ResponseTxt: flow.Response.GetData(),
					// Request: &Flow_HttpRequest{
