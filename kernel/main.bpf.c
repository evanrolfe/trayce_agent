...
static __always_inline int gotls_read_ex(struct pt_regs *ctx, bool is_register_abi) {
    ...
    if (active_buf_t != NULL) {
        ...
        // bpf_printk("!!!!!!!!!!!!!!!!!!!!!!!!!!> pid: %d, fd: %d, buf_len: %d", pid, fd, active_buf_t->buf_len);
        ...
        // This is a max function, but it is written in such a way to keep older BPF verifiers happy.
        event->data_len = (active_buf_t->buf_len < MAX_DATA_SIZE_OPENSSL ? (active_buf_t->buf_len & (MAX_DATA_SIZE_OPENSSL - 1)) : MAX_DATA_SIZE_OPENSSL);
        ...
        bpf_ringbuf_output(&data_events, event, sizeof(struct data_event_t), 0);
        bpf_printk("gotls/read ex: sent event %d len: %d end", pid, event->data_len);
    }
...
SEC("uprobe/gotls_exit_write_register")
int probe_exit_go_tls_write(struct pt_regs* ctx) {
    u64 current_pid_tgid = bpf_get_current_pid_tgid();
    u32 pid = current_pid_tgid >> 32;
    bpf_printk("=====> Exit: gotls/write pid: %d\n", pid);
...
int probe_entry_go_tls_read(struct pt_regs* ctx) {
    ...
    bpf_printk("=====> Enter: gotls/read pid: %d", pid);
    gotls_read(ctx, true);
...
int probe_exit_go_tls_read(struct pt_regs* ctx) {
    ...
    bpf_printk("=====> Exit: gotls/read pid: %d\n", pid);
